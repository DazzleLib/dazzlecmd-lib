...
- `self_setup` module -- the aggregator's own PATH bootstrap (dazzlecmd#103). `setup` at aggregator level means "make ME reachable": after a user-scheme `pip install` on Windows, console-script shims land in the per-version `%APPDATA%\Python\Python3XY\Scripts` directory that Windows never adds to PATH, so the aggregator command is not found in any shell. `diagnose()` reports install scheme, scripts dir, shim presence, effective + persisted PATH membership, and shadowing copies; `fix_windows()` appends the verified scripts dir to the persisted user PATH via `winreg` preserving the registry value kind (never `setx`, which truncates at 1024 chars), writes a timestamped backup under `~/.dazzlecmd/`, and broadcasts `WM_SETTINGCHANGE`; `advise_posix()` prints the exact rc-file line without editing dotfiles; `first_run_hint()` gives `__main__` a one-line pointer when (and only when) the bootstrap would help; `run_self_setup()` is the shared diagnose-explain-prompt flow (`--yes` / `--dry-run` semantics, TTY-aware). Registry access sits behind monkeypatchable seams so the whole flow is testable on any platform. A venv-local install gets activation guidance and is NEVER offered a persistent PATH append (a venv's Scripts dir on the user PATH would rot when the venv dies); a tool shadowing the aggregator's own name is surfaced with a pointer to its FQCN spelling; an orphaned `--` tail (no target) warns instead of silently dropping. The PATH-value comparators delegate to `dazzle_filekit.pathenv` (new floor: dazzle-filekit>=0.3.3), keeping the original windows-bool adapters for callers.
- `shell_fix` module -- heal the CURRENT shell without opening a new one. A child process cannot edit its parent shell's environment, but a script the shell interprets in-process can (conda-activate architecture): self-setup now writes idempotent per-shell fix scripts (`<brand>-path.cmd` / `.ps1` / `.sh`) to the temp dir and prints the one invocation line for the DETECTED invoking shell (parent-process walk via Toolhelp32, MSYS/`$SHELL` fingerprints; unknown -> all dialects, labeled), clipboard-loading it when possible. The per-shell nuance is enforced and tested: cmd runs the batch in-process, PowerShell/POSIX forms MUST be dot-sourced/sourced (plain-running mutates a child scope only -- pinned by a negative test). `setup <self> --emit-shell-fix` is the machine channel: stdout is exactly the invocation line, diagnostics on stderr. Live-verified in real cmd and PowerShell sessions.
- `paths.which_in_dir` / `paths.which_all_on_path` -- the single-directory and all-hits siblings of `which_with_pathext` (PATHEXT-aware on Windows, executable-bit-aware on POSIX). Homed in `paths.py` as the executable-lookup surface; `self_setup` delegates to them for shim verification and shadow-copy detection.

- `verb_contracts` module -- the argument-contract continuum a verb subscribes to (dazzlecmd#104). Bare dispatch passes everything through; verb-mediated (`<verb> <target> <verb-params> -- <level-params>`) gives the verb the space before `--` and the target everything after. `setup` subscribes to verb-mediated; the engine's meta path now splits the first `--` BEFORE argparse (closing the documented-since-v0.7.46-but-never-wired forwarding gap: trailing args were rejected as "unrecognized arguments"), and the default setup handler forwards the level-args into the tool's invocation with host-correct shell quoting (`join_for_shell`). The subscription registry is the source of truth for the future `:.meta:verb:<verb>.contract` derived property.

### Changed

- Executable lookup goes through one door: `conditions._command_available` and `registry`'s runtime-availability checks (interpreter / npx / npm) now call `paths.which_with_pathext` instead of raw `shutil.which` -- behavior-identical, but the lib's own wrapper policy is now consistent (stack-survey B7).
- The default `setup` verb gained `--yes` / `--dry-run` as verb-level params (preview-and-consent for every setup target) and the aggregator self-target routing (see `self_setup` above).
...
## [0.10.6-alpha] - 2026-07-02

The path-form intercept (3d") -- the CLI's new FIRST routing rule, single-sited in `engine.run()` above the registry/escape-hatch fork so both run paths share one implementation. An operator-led first non-flag token is classified by the locked mnemonic -- **dot anywhere = look, all-colons = run**: `.`/`:.`-led paths route to the property surface (bare = get, one value token = upsert with the added/updated echo; trailing `:.` = the boundary-aware plane listing; property forms return BEFORE tool discovery -- they don't need it); a pure-`:` entity path strips its leading colon and rejoins normal dispatch (`dz :core:safedel` invokes it -- root elision for the entity plane); `:+`-led is a reserved one-liner (supra lands with SD-7). Value rules: a bare negative number is a value by argparse's own matcher (`dz :.kit.channels.verbosity -3` just works); other `-`-led values need `--`; multi-word values must be quoted; a literal `--` is not settable. A leading bare `--` disables the intercept (POSIX). Pre-path global flags are collected and handed to the aggregator's `sugar_flags_hook` (so `dz -v .note` honors `-v`); `--show`'s separated value is skipped correctly. prop_commands now renders empty-value errors uniformly (exit 2) instead of raising.
...
## [0.10.3-alpha] - 2026-07-02

Path semantics + value helpers (3a2"). `fqcn_grammar.canonicalize()` implements the first-segment FIBER_ROOTS forgiveness rule: a `:.`-led path whose first segment is not fiber-plane vocabulary (the exported `FIBER_ROOTS` frozenset -- `level` deliberately excluded: it is the root property) rewrites its leading `:.` to `.` at the TEXT level and re-parses -- so `:.note.author` canonicalizes whole to `.note.author` (sub-keys and case preserved: `:.env-vars:DEBUG` -> `.env-vars:DEBUG`), while `:.kit.channels.verbosity` passes verbatim and interior `:.`/`:+` after a forgiven segment error rather than guess. New `property_values` module: `is_negative_number_token()` pinned to argparse's own matcher (verified against the live parser in tests, so CPython drift fails loudly -- `-3`, `-0.5`, and `-.5` are values; `-1e5`/`-inf` need `--`) and `parse_property_value()` (counted `vvvv`/`qqq`, ints/floats, caller-supplied named ranks, JSON literals; an empty token errors with a `prop delete` hint since PowerShell 5.1 drops empty arguments). `PropertyStore.list_prefix` is now BOUNDARY-AWARE via the new `key_in_family()` -- `dz:.kit` no longer over-matches `dz:.kitchen.note` (shared-stem siblings); the empty prefix still lists everything.
